...
export async function findColorFunctionsInText(text: string): Promise<ColorMatch[]> {
...
function createColorFunctionObject(match: RegExpMatchArray): ColorMatch {
  ...
  if (cssVarMatchArray.length > 0) {
    const cssVarMatch = cssVarMatchArray[0] as RegExpMatchArray;
...
export function sortStringsInDescendingOrder(strings: string[]): string[] {
